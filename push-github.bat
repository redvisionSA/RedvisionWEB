...
REM    2. Define la identidad del autor SOLO para este repositorio.
REM    3. Agrega todo lo que NO este excluido por .gitignore.
REM       Quedan fuera: node_modules, dist, public/draco (se regenera en
REM       cada npm install) e img (material crudo, ~48 MB).
REM    4. Crea el primer commit.
REM    5. Lo sube a https://github.com/redvisionSA/RedvisionWEB.git
REM
...
echo.
echo === Identidad del autor (solo para este repositorio)...
git config user.name "TECNICO REDVISION" || goto :error
git config user.email "tecnol.redvision@gmail.com" || goto :error
